...
def rendering_map(stdscr, player_x, player_y):
    ...
    offset_x = max(0, min(player_x - level.view_height // 2, level.height - level.view_height))
    offset_y = max(0, min(player_y - level.view_width // 2, level.width - level.view_width))

    # Loop through each cell in the viewport
    for x in range(level.view_height):
        for y in range(level.view_width):
            map_x = offset_x + x  # Map coordinate for current row
            map_y = offset_y + y  # Map coordinate for current column
    ...
    if 0 <= screen_x < level.view_height and 0 <= screen_y < level.view_width:
        stdscr.addstr(screen_x, screen_y + master_offset, '@', curses.color_pair(4))
    if fog_of_war:
        # for x in range(3):
        #     for y in range(3):
        #         if 0 <= screen_y + y < level.view_height and 0 <= screen_x + x < level.view_width:
        #             level.visible[player_y + y - 1][player_x + x - 1] = ''
        levelManager.calculate_field_of_view(player_x, player_y, 20)
