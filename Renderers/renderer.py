    ...
    for y in range(level.view_height):
        for x in range(level.view_width):
            ...
            if 0 <= map_y < level.height and 0 <= map_x < level.width:
                ...
                if tile == '#':
                    stdscr.addstr(y, x + master_offset, tile, curses.color_pair(2))  # Wall
                elif tile == '+' or tile == '`':
                    stdscr.addstr(y, x + master_offset, tile, curses.color_pair(3))  # Door
                elif tile == '.':
                    stdscr.addstr(y, x + master_offset, tile, curses.color_pair(1))  # Floor
                else:
    ...
    if fog_of_war:
        ...
        for y in range(level.view_height):
            for x in range(level.view_width):
                ...
                map_x = offset_x + x  # Map coordinate for current column (x)
                if 0 <= map_y < level.height and 0 <= map_x < level.width:
                    stdscr.addstr(y, x + master_offset, level.visible[map_y][map_x], curses.color_pair(1))
