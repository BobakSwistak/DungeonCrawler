...
def render_level(terminal, player_y, player_x):
    ...
    offset_y = max(0, min(player_y - level_init.view_height // 2, level_init.height - level_init.view_height))
    offset_x = max(0, min(player_x - level_init.view_width // 2, level_init.width - level_init.view_width))
    if not player.menu_opened:

        if level_init.fog_of_war:
...
def render_map(terminal, player_y, player_x):
    ...
    for y in range(level_init.view_height):
        for x in range(level_init.view_width):
            ...
            if 0 <= map_y < level_init.height and 0 <= map_x < level_init.width:
                tile = level.current_level.memorized[map_y][map_x]
        ...
        for x in range(level_init.view_width):
            ...
            if 0 <= map_y < level_init.height and 0 <= map_x < level_init.width:
                if level.current_level.visible[map_y][map_x]:
                    tile = level.current_level.level[map_y][map_x]
                    if tile == '#' or tile == '.':
    ...
    if 0 <= screen_y < level_init.view_height and 0 <= screen_x < level_init.view_width:
...
def render_fog_of_war(terminal, player_y, player_x):
    # for y in range(3):
    #     for x in range(3):
    #         if 0 <= screen_y + y < level_init.view_height and 0 <= screen_x + x < level_init.view_width:
...
def render_enemies(terminal):
    for enemy in enemies.enemies_list:
        if level.current_level.visible[enemy.enemy_pos[0]][enemy.enemy_pos[1]]:
            ...
            if offset_y <= enemy_y < offset_y + level_init.view_height and offset_x <= enemy_x < offset_x + level_init.view_width:
