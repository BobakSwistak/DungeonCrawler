...
def player_input(stdscr, player_x, player_y, level):
    ...
    if key != None:
        ...
        if level.can_move:
            ...
            new_x = player_x + dx
            new_y = player_y + dy
            if 0 <= new_x < level.height and 0 <= new_y < level.width:
                if level.level[new_x][new_y] in level.walkable:
                    player_x, player_y = new_x, new_y
                elif level.level[new_x][new_y] == "+":
                    level.level[new_x][new_y] = "`"  # Open the door
            return player_x, player_y
