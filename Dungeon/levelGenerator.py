import random
import Enemies.enemyManager as enemyManager

from Dungeon import level, levelInit
from Resources.tiles import Tiles


class LevelGenerator:

    def __init__(self):
        self.local_level = []
        self.local_occupied = []
        self.local_visible = []
        self.local_memorized = []
        self.local_rooms = []

    def generate_dungeon(self):
        # Initialize level and visible arrays as [y][x]
        self.local_level = [[Tiles.wall for _ in range(levelInit.width)] for _ in range(levelInit.height)]
        self.local_occupied = [[False for _ in range(levelInit.width)] for _ in range(levelInit.height)]
        self.local_visible = [[Tiles.void for _ in range(levelInit.width)] for _ in range(levelInit.height)]
        self.local_memorized = [[Tiles.void for _ in range(levelInit.width)] for _ in range(levelInit.height)]
        self.carve_room()

        self.generate_rooms()
        self.generate_tunnels()
        self.generating_doors()
        self.generate_staircases()
        self.clean_up()
        # Spawn player in the center room
        center_y = levelInit.height // 2 + 3
        center_x = levelInit.width // 2 + 3
        self.export_level()
        self.generate_enemies()
        starter_room = random.choice(self.local_rooms)
        return (starter_room[0] + starter_room[2]) // 2, (starter_room[1] + starter_room[3]) // 2  # (y, x)

    def generate_rooms(self):
        for _ in range(levelInit.roomCount):
            minDistance = 9999
            nearestPlace = None
            room = self.carve_room()
            if room is None:
                continue
            for y in range(levelInit.height):
                for x in range(levelInit.width):
                    if self.local_level[y][x] == Tiles.floor:
                        if room[2] > y > room[0] and room[3] > x > room[1]:
                            continue
                        distance = ((y - room[0] + 3) ** 2 + (x - room[1] + 3) ** 2) ** 0.5
                        if distance < minDistance:
                            minDistance = distance
                            nearestPlace = (y, x)
            startPos = (room[0] + room[2]) // 2, (room[1] + room[3]) // 2  # (y, x)
            endPos = nearestPlace
            self.carve_tunnel(startPos, endPos)

    def generate_tunnels(self):
        for room in self.local_rooms:
            minDistance = 9999
            roomCenter = [(room[0] + room[2]) // 2, (room[1] + room[3]) // 2]  # (y, x)
            nearestRoom = None
            for other in self.local_rooms:
                if room != other:
                    otherCenter = [(other[0] + other[2]) // 2, (other[1] + other[3]) // 2]
                    distance = ((otherCenter[0] - roomCenter[0]) ** 2 + (otherCenter[1] - roomCenter[1]) ** 2) ** 0.5
                    if distance < minDistance:
                        minDistance = distance
                        nearestRoom = otherCenter
            if nearestRoom:
                self.carve_tunnel(roomCenter, nearestRoom)

    def carve_room(self, roomH=None, roomW=None, y=None, x=None):
        if roomH is None:
            roomH = random.randint(levelInit.roomSize[0], levelInit.roomSize[1])
            roomW = random.randint(levelInit.roomSize[0], levelInit.roomSize[1])
            y = random.randint(1, levelInit.height - roomH - 2)
            x = random.randint(1, levelInit.width - roomW - 2)

        for j in range(roomH):
            for i in range(roomW):
                check_y = y + j
                check_x = x + i
                if check_y >= levelInit.height or check_x >= levelInit.width:
                    return None
                if self.local_level[check_y][check_x] != Tiles.wall:
                    return None
        for j in range(roomH):
            for i in range(roomW):
                self.local_level[y + j][x + i] = Tiles.wall

        for j in range(roomH - 2):
            for i in range(roomW - 2):
                self.local_level[y + j + 1][x + i + 1] = Tiles.floor
        room = [y, x, roomH + y, roomW + x]  # [y1, x1, y2, x2]
        self.local_rooms.append(room)
        return room

    def carve_tunnel(self, startPos, endPos):
        # startPos and endPos are (y, x)
        if startPos[0] < endPos[0]:
            for y in range(startPos[0], endPos[0] + 1):
                self.local_level[y][startPos[1]] = Tiles.floor
        else:
            for y in range(endPos[0], startPos[0] + 1):
                self.local_level[y][startPos[1]] = Tiles.floor
        if startPos[1] < endPos[1]:
            for x in range(startPos[1], endPos[1] + 1):
                self.local_level[endPos[0]][x] = Tiles.floor
        else:
            for x in range(endPos[1], startPos[1] + 1):
                self.local_level[endPos[0]][x] = Tiles.floor

    def carve_door(self, y, x):
        if random.random() < levelInit.open_door_chance:
            self.local_level[y][x] = Tiles.open_door
        elif random.random() < levelInit.trapped_door_chance:
            self.local_level[y][x] = Tiles.trapped_door
        else:
            self.local_level[y][x] = Tiles.closed_door

    def generating_doors(self):
        for y in range(1, levelInit.height - 1):
            for x in range(1, levelInit.width - 1):
                if self.local_level[y][x] == Tiles.floor:
                    if ((self.local_level[y - 1][x] == Tiles.wall and self.local_level[y + 1][x] == Tiles.wall and
                         self.local_level[y][x - 1] == Tiles.floor and self.local_level[y][x + 1] == Tiles.floor) or
                            (self.local_level[y][x - 1] == Tiles.wall and self.local_level[y][x + 1] == Tiles.wall and
                             self.local_level[y - 1][x] == Tiles.floor and self.local_level[y + 1][x] == Tiles.floor)):
                        if (self.local_level[y + 1][x + 1] == Tiles.floor or self.local_level[y + 1][
                            x - 1] == Tiles.floor or
                                self.local_level[y - 1][x + 1] == Tiles.floor or self.local_level[y - 1][
                                    x - 1] == Tiles.floor):
                            self.carve_door(y, x)
                if self.local_level[y][x] == Tiles.wall:
                    if ((self.local_level[y - 1][x] == Tiles.floor and self.local_level[y + 1][x] == Tiles.floor and
                         self.local_level[y][x - 1] == Tiles.wall and self.local_level[y][x + 1] == Tiles.wall) or
                            (self.local_level[y][x - 1] == Tiles.floor and self.local_level[y][x + 1] == Tiles.floor and
                             self.local_level[y - 1][x] == Tiles.wall and self.local_level[y + 1][x] == Tiles.wall)):
                        if random.random() < levelInit.random_door_chance:
                            self.carve_door(y, x)
                            if random.random() < levelInit.hidden_door_chance:
                                self.local_level[y][x] = Tiles.hidden_door
                            else:
                                self.local_level[y][x] = Tiles.closed_door

    def clean_up(self):
        for y in range(1, levelInit.height - 1):
            for x in range(1, levelInit.width - 1):
                if all(self.local_level[yy][xx] in [Tiles.wall, Tiles.void] for yy in range(y - 1, y + 2) for xx in
                       range(x - 1, x + 2)): self.local_level[y][x] = Tiles.void

        for y in range(levelInit.height):
            self.local_level[y][0] = Tiles.void
            self.local_level[y][levelInit.width - 1] = Tiles.void
        for x in range(levelInit.width):
            self.local_level[0][x] = Tiles.void
            self.local_level[levelInit.height - 1][x] = Tiles.void

    def generate_enemies(self):
        for i in range(random.randint(10, 10)):
            enemyManager.generate_enemy("Skeleton")

    def generate_staircases(self):
        for i in range(levelInit.staircase_down_count):
            self.generate_down_staircase()
        for i in range(levelInit.staircase_up_count):
            self.generate_up_staircase()

    def generate_up_staircase(self):
        y = random.randint(1, levelInit.height - 1)
        x = random.randint(1, levelInit.width - 1)
        if self.local_level[y][x] == Tiles.floor:
            self.local_level[y][x] = Tiles.staircase_up
        else:
            self.generate_up_staircase()

    def generate_down_staircase(self):
        y = random.randint(1, levelInit.height - 1)
        x = random.randint(1, levelInit.width - 1)
        if self.local_level[y][x] == Tiles.floor:
            self.local_level[y][x] = Tiles.staircase_down
        else:
            self.generate_down_staircase()

    def export_level(self):
        level.current_level.level = self.local_level
        level.current_level.occupied = self.local_occupied
        level.current_level.visible = self.local_visible
        level.current_level.memorized = self.local_memorized
        level.current_level.rooms = self.local_rooms


levelGenerator = LevelGenerator()
